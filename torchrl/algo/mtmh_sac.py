...
class MTMHSAC(MTSAC):
    ...
    def update(self, batch):
        ...
        means = unsqe_cat_gather(mean_list, task_idx, dim = 1)

        log_stds = unsqe_cat_gather(log_std_list, task_idx, dim = 1)

        new_actions = unsqe_cat_gather(new_actions_list, task_idx, dim = 1)

        # log_probs = torch.cat
        log_probs = unsqe_cat_gather(log_probs_list, task_idx, dim = 1)
        ...
        q1_preds = unsqe_cat_gather(q1_pred_list, task_idx, dim = 1)
        q2_preds = unsqe_cat_gather(q2_pred_list, task_idx, dim = 1)
        ...
        with torch.no_grad():
            ...
            target_actions = unsqe_cat_gather(target_actions_list, task_idx, dim = 1)

            target_log_probs_list = target_sample_info["log_prob"]
            target_log_probs = unsqe_cat_gather(target_log_probs_list, task_idx, dim = 1)
            ...
            target_q1_pred = unsqe_cat_gather(target_q1_pred_list, task_idx, dim = 1)
            target_q2_pred = unsqe_cat_gather(target_q2_pred_list, task_idx, dim = 1)
        ...
        q1_new_actions = unsqe_cat_gather(q1_new_actions_list, task_idx, dim = 1)
        q2_new_actions = unsqe_cat_gather(q2_new_actions_list, task_idx, dim = 1)
